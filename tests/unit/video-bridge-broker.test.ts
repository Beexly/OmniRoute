...
test("the module-level broker singleton queue is the same object for both frame and audio dispatch", async () => {
  // Neither branch of the route is given a `queue` dependency here, so both
  // must fall back to the identical `extractionQueue` singleton — proving
  // production traffic for frames and audio share one process queue.
  ...
  assert.equal(
    ...
    2,
    "both the frame and audio branches must default to the one extractionQueue singleton"
  );
});
